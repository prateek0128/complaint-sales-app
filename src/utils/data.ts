...
export function pickList(data: unknown): Complaint[] {
  ...
  for (const candidate of candidates) {
    if (Array.isArray(candidate)) return candidate as Complaint[];
    if (candidate && typeof candidate === "object") {
      const nested = Object.values(candidate as Record<string, unknown>).find(Array.isArray);
      if (Array.isArray(nested)) return nested as Complaint[];
    }
  }
  return Array.isArray(data) ? (data as Complaint[]) : [];
}

export function pickObject<T extends Record<string, unknown>>(data: unknown): T {
  const value = data as Record<string, unknown>;
  if (value.data && typeof value.data === "object" && !Array.isArray(value.data)) return value.data as T;
  return value as T;
}
